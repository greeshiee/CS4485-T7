...
const UseCase1: React.FC = () => {
    ...
    const runButtonClick = (i: number) => {
        fetch('http://localhost:8000/run', {  // Ensure this URL matches your FastAPI server
            method: 'POST',
            headers: {
        ...
        .then(data => {
            setOutput(data.stdout);  // Set the output from FastAPI
        })
        .catch(error => {
    ...
    const addButtonClick = () => {
        setCodeBlocks([...codeBlocks, '']);  // Add a new code block
    };

    const removeButtonClick = (i: number) => {
        const newCodeBlocks = codeBlocks.filter((_, index) => index !== i);
        setCodeBlocks(newCodeBlocks);  // Remove the code block at index i
    };
            ...
            <div className="flex flex-row h-full">
                ...
                <section className="w-1/3 p-4">
                    ...
                    <form className="upload-file" onSubmit={(e) => {
                        ...
                        //uploadFile(formData);  // Implement this function to handle file uploads
                    }}>
